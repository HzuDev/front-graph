import { databases, storage, DATABASE_ID, COLLECTIONS, GEOJSON_BUCKET_ID, Query } from '../appwrite';
 ...
 */
function parseGeoJsonCoordinates(geoJsonData: any, claimId: string): number[][][] | null {
 ...
 */
async function fetchPolygonCoordinates(claim: Claim): Promise<number[][][] | null> {
    try {
        let coordinates;
        
        // Check if value_raw is a URL (Appwrite Storage link)
        if (typeof claim.value_raw === 'string' && claim.value_raw.startsWith('http')) {
            ...
            const urlMatch = claim.value_raw.match(/\/buckets\/([^\/]+)\/files\/([^\/]+)\//);
...
export async function fetchPolygons(): Promise<PolygonData[]> {
    try {
        ...
        for (const claim of response.documents) {
            ...
            if (claim.value_raw && claim.subject) {
                try {
                    ...
                    const entityId = typeof claim.subject === 'string' ? claim.subject : claim.subject.$id;
                    ...
                    polygons.push({
                        entityId,
                        entityLabel: typeof claim.subject === 'string' ? '' : claim.subject.label || '',
        ...
        const polygonsNeedingLabels = polygons.filter(p => !p.entityLabel || p.entityLabel === '');
        if (polygonsNeedingLabels.length > 0) {
            try {
                // Fetch entities in batches to get their labels
                const entityIds = [...new Set(polygonsNeedingLabels.map(p => p.entityId))];
 ...
 * This is a simple point-in-polygon check
 */
export async function findMunicipalityByCoordinates(lat: number, lon: number): Promise<Entity | null> {
    try {
