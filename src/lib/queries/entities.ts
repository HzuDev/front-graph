...
import { inferEntityTypeFromLabel, type EntityType } from '../appwrite/entity-utils';
 ...
 */
function deriveEntityTypeFromClaims(claims: Claim[], entityLabel?: string): EntityType {
    ...
    for (const instanceClaim of instanceClaims) {
        ...
        if (typeId === ENTITY_TYPE_IDS.TERRITORIO || typeId === ENTITY_TYPE_IDS.ENTIDAD_TERRITORIAL) return 'TERRITORIO';
        if (typeId === ENTITY_TYPE_IDS.PARTIDO_POLITICO) return 'PARTIDO_POLITICO';
        if (typeId === ENTITY_TYPE_IDS.CASA_ENCUESTADORA) return 'CASA_ENCUESTADORA';
        if (
        ...
        const typeLabel = typeof instanceClaim.value_relation === 'object' ? instanceClaim.value_relation?.label : undefined;
        if (typeLabel) {
    ...
    try {
        ...
        if (claimIds.length > 0) {
            ...
            for (let i = 0; i < claimIds.length; i += batchSize) {
                ...
                qualifiersResponse.documents.forEach((qualifier) => {
                    const claimId = typeof qualifier.claim === 'object' ? qualifier.claim.$id : qualifier.claim;
                    if (!claimId) return;
                    if (!qualifiersByClaim.has(claimId)) qualifiersByClaim.set(claimId, []);
                    qualifiersByClaim.get(claimId)?.push(qualifier);
                });

                referencesResponse.documents.forEach((reference) => {
                    const claimId = typeof reference.claim === 'object' ? reference.claim.$id : reference.claim;
                    if (!claimId) return;
                    if (!referencesByClaim.has(claimId)) referencesByClaim.set(claimId, []);
        ...
        allClaims.forEach(claim => {
            ...
            qualifiers.forEach((qualifier) => {
                ...
                if (qualifier.value_relation && typeof qualifier.value_relation === 'string') {
        ...
        if (entityIdsToFetch.size > 0) {
            ...
            for (const claim of allClaims) {
                ...
                if (typeof claim.subject === 'string' && fetchedEntities.has(claim.subject)) {
                    claim.subject = fetchedEntities.get(claim.subject);
                }

                (claim.qualifiers || []).forEach((qualifier) => {
                    if (typeof qualifier.property === 'string' && fetchedEntities.has(qualifier.property)) {
                        qualifier.property = fetchedEntities.get(qualifier.property);
                    }
                    if (typeof qualifier.value_relation === 'string' && fetchedEntities.has(qualifier.value_relation)) {
                        qualifier.value_relation = fetchedEntities.get(qualifier.value_relation);
                    }
                });

                (claim.references || []).forEach((reference) => {
                    if (typeof reference.reference === 'string' && fetchedEntities.has(reference.reference)) {
        ...
        const outgoingClaims = allClaims.filter(
            (c) =>
                (typeof c.subject === 'object' ? c.subject?.$id : c.subject) === entityId
        );
