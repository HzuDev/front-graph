...
import { DB_ID, COLLECTION_IDS, PROPERTY_IDS, ENTITY_TYPE_IDS } from "@/lib/constants/entity-types";


export type EntityType = "PERSONA" | "POLITICO" | "TERRITORIO" | "INSTITUCION" | "PARTIDO_POLITICO" | "ENCUESTA" | "CASA_ENCUESTADORA" | "EDUCACION" | "ROL" | "UNKNOWN";
...
export function inferEntityTypeFromLabel(label?: string): EntityType {
    ...
    if (SURVEY_LABEL_HINTS.some((hint) => normalized.includes(hint))) return "ENCUESTA";
    if (EDUCATION_LABEL_HINTS.some((hint) => normalized.includes(hint))) return "EDUCACION";
    if (INSTITUTION_LABEL_HINTS.some((hint) => normalized.includes(hint))) return "INSTITUCION";
    if (TERRITORY_LABEL_HINTS.some((hint) => normalized.includes(hint))) return "TERRITORIO";
    return "UNKNOWN";
}
 ...
 */
export async function determineEntityType(entityId: string, databases: Databases): Promise<EntityType> {
    try {
        ...
        if (claims.total > 0) {
            for (const claim of claims.documents) {
                ...
                if (typeId === ENTITY_TYPE_IDS.MINISTERIO || typeId === ENTITY_TYPE_IDS.ORGANO_ELECTORAL || typeId === ENTITY_TYPE_IDS.ASAMBLEA_LEGISLATIVA) return "INSTITUCION";
                
                let typeLabel = typeof claim.value_relation === 'object' ? claim.value_relation.label : "";
                
                if (!typeLabel && typeof typeId === 'string') {
                    try {
                        const relDoc = await databases.getDocument(DB_ID, COLLECTION_IDS.ENTITIES, typeId);
        ...
        const entity = await databases.getDocument(DB_ID, COLLECTION_IDS.ENTITIES, entityId);
