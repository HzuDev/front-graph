...
type SearchPageAction =
  | { type: 'INITIALIZE'; payload: { q: string; department: string; type: string } }
...
const searchPageReducer = (state: SearchPageState, action: SearchPageAction): SearchPageState => {
  switch (action.type) {
    case 'INITIALIZE':
      return {
        ...state,
...
const EntityCard = memo<{ entity: Entity }>(({ entity }) => {
  const inferredType = entity.label ? inferEntityTypeFromLabel(entity.label) : "UNKNOWN";
        ...
        <div>
          ...
          {entity.description ? (
            ...
            <div className="flex items-start gap-3 bg-primary-green/5 p-4 rounded-2xl mt-4">
              <Activity size={16} className="mt-0.5 text-primary-green/40 shrink-0" />
              <p className="text-xs font-medium text-primary-green/60 leading-relaxed">
                Haz clic para explorar los datos, declaraciones y otros vínculos asociados en nuestra base de datos.
              </p>
            </div>
          )}
        </div>
...
const SearchPage: React.FC<SearchPageProps> = ({
  ...
  useEffect(() => {
    if (state.isInitialized && typeof window !== "undefined") {
      ...
      if (state.debouncedSearch) url.searchParams.set("q", state.debouncedSearch);
      else url.searchParams.delete("q");

      if (state.selectedDepartment !== DEFAULT_DEPARTMENT) url.searchParams.set("department", state.selectedDepartment);
      else url.searchParams.delete("department");

      if (state.selectedType !== DEFAULT_TYPE) url.searchParams.set("type", state.selectedType);
      else url.searchParams.delete("type");

      window.history.replaceState({}, "", url.toString());
    }
  }, [state.debouncedSearch, state.selectedDepartment, state.selectedType, state.isInitialized]);
  ...
  useEffect(() => {
    ...
    loadData();
  }, [state.selectedDepartment, state.selectedType, state.debouncedSearch, state.isInitialized]);

  const paginationData = useMemo(() => {
    const totalPages = Math.max(1, Math.ceil(state.entities.length / ITEMS_PER_PAGE));
      ...
      <div className="max-w-7xl mx-auto px-6 mt-36">
        <div className="flex flex-col md:flex-row justify-between items-end  gap-8 border-b border-primary-green/5 pb-12">
          <div className="space-y-4">
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-none">
              Hay <span className="text-transparent bg-clip-text bg-linear-to-r from-primary-green to-primary-green/40 italic">{state.loading ? "..." : state.entities.length}</span> coincidencias
            </h2>
            {state.debouncedSearch && (
              <p className="text-lg font-medium text-primary-green/40">
                Mostrando resultados para la búsqueda de <span className="text-primary-green font-bold">"{state.debouncedSearch}"</span>
              </p>
            )}
          </div>
          <div className="flex p-1.5 bg-primary-green/5 rounded-[1.5rem] border border-primary-green/5 overflow-x-auto w-full md:w-auto hidescrollbar shrink-0">
            {QUICK_FILTERS.map(f => (
              <button
                key={f.label}
                onClick={() => dispatch({ type: 'SET_SELECTED_TYPE', payload: f.type })}
          ...
          {state.showFilters && (
            <div className="max-w-7xl mx-auto mt-6 p-6 bg-white border border-primary-green/5 rounded-3xl shadow-sm flex flex-col md:flex-row gap-6 animate-[fadeInUp_0.3s_cubic-bezier(0.16,1,0.3,1)_forwards]">
              <div className="flex-1">
                <label htmlFor="filter-department" className="text-[10px] font-black uppercase tracking-widest opacity-40 mb-3 block">
                  Departamento
                </label>
                <select
                  id="filter-department"
                  value={state.selectedDepartment}
                  onChange={(e) => dispatch({ type: 'SET_SELECTED_DEPARTMENT', payload: e.target.value })}
              ...
              <div className="flex-1">
                <label htmlFor="filter-type" className="text-[10px] font-black uppercase tracking-widest opacity-40 mb-3 block">
                  Tipo de Entidad
                </label>
                <select
                  id="filter-type"
        ...
        {state.loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={`loading-skeleton-${i}`} className="bg-white border border-primary-green/5 p-8 rounded-[3.5rem] min-h-80 animate-pulse"></div>
            ))}
          </div>
        ) : state.entities.length === 0 ? (
          <div className="py-20 text-center">
            <Search size={64} className="mx-auto mb-6 text-primary-green/10" />
            <h3 className="text-3xl font-black text-primary-green mb-4">No se encontraron resultados</h3>
            <p className="text-primary-green/40 font-medium max-w-md mx-auto mb-8">No hemos podido encontrar ninguna entidad que coincida con tu búsqueda. Prueba usando otros términos o limpiando los filtros.</p>
            <button
