...
export function RoleView({ entity, claims = EMPTY_CLAIMS }: RoleProps) {
    ...
    const descripcion = entity.description || "Responsable de la gestión administrativa y fiscalización.";
    ...
    const alcance = jurisdictionClaim?.value_relation?.label || jurisdictionClaim?.value_raw || "Nacional";

    const eventClaim = claims.find((c) => {
        const prop = c.property?.label?.toLowerCase() || "";
        return prop.includes("evento") || prop.includes("eleccion") || prop.includes("elección");
    });
    const proximoEvento = eventClaim?.value_relation?.label || eventClaim?.value_raw || "Subnacionales 2026";


    const holderClaims = claims.filter((c) => {
        const prop = c.property?.label?.toLowerCase() || "";
        return (prop.includes("titular") || prop.includes("persona") || prop.includes("ocupa") || prop.includes("ejerce") || prop.includes("candidato"));
    });

    const candidatos = holderClaims.map(c => {

        const roleId = entity.$id;
        const outRelationId = typeof c.value_relation === 'object' ? c.value_relation?.$id : c.value_relation;
        ...
        return {
            ...
            partido: "Candidato Político", // Consider extracting more data if we had the person's claims
            link: candidateEntity?.$id ? buildPath(`/entity?id=${candidateEntity.$id}`) : "#"
        };
        ...
        <main className="min-h-screen bg-neutral-white text-primary-green font-sans antialiased pb-24">
            <section className="bg-primary-green pt-32 pb-20 px-6 relative overflow-hidden">
                ...
                <div className="max-w-6xl mx-auto relative z-10">
                    <div
                        className="flex flex-col md:flex-row gap-10 items-center md:items-end"
                    >
                        <div
                            className="w-24 h-24 bg-hunter rounded-[2rem] flex items-center justify-center text-primary-green shadow-2xl rotate-3 border-4 border-white/10 shrink-0"
                        >
                        ...
                        <div className="flex-1 text-center md:text-left text-hunter">
                            <div
                                className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 rounded-full border border-white/10 mb-4 backdrop-blur-md"
                            >
                                <span
                            ...
                            <p
                                className="text-lg md:text-xl text-hunter/50 font-medium max-w-2xl text-pretty leading-relaxed mx-auto md:mx-0"
                            >
                                {descripcion}
                            </p>
                        </div>
            ...
            >
                <div
                    className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-primary-green/5 flex items-center gap-5"
                >
                    ...
                    <div>
                        ...
                        <p className="text-2xl font-black">
                            {totalRegistrados}{typeof totalRegistrados === 'number' ? '+' : ''}
                        </p>
                    </div>
                </div>
                <div
                ...
                <aside className="lg:col-span-4 space-y-8">
                    ...
                    >
                        <div className="relative z-10">
                            ...
                            <p
                                className="text-sm font-bold text-primary-green/70 leading-relaxed text-pretty"
                            >
                                En el sistema electoral boliviano, el <span
                                    className="text-primary-green">{titulo}</span> ejerce funciones ejecutivas de acuerdo con la Constitución
                                Política del Estado.
                            </p>
                        </div>
                    </div>
                    ...
                    >
                        <h4
                            className="text-2xl font-black tracking-tighter mb-6 relative z-10" style={{ textWrap: "balance" }}
                        >
                        ...
                        <div className="space-y-4 relative z-10">
                            ...
                            >
                                <p
                                    className="text-[9px] font-black uppercase tracking-widest opacity-40 mb-1"
                                >
                                    ID Único de Cargo
                                </p>
                                <p
                                    className="text-xs font-mono font-bold break-all opacity-80"
                                >
                                    {entity.$id}
                                </p>
                            </div>
                            <div
                                className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest opacity-40"
                            >
