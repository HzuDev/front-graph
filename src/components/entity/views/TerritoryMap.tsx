...
const TerritoryMapContent = lazy(() =>
  import('./TerritoryMapContent').then((m) => ({
    default: m.TerritoryMapContent,
  }))
);

const normalizeGeoJson = (data: unknown) => {
  if (!data) return null;
  ...
  if (obj.type === 'Polygon' || obj.type === 'MultiPolygon') {
    return { type: 'Feature', geometry: obj } as unknown as GeoJsonObject;
  }
...
const useTerritoryGeoData = (
  ...
  const { data, error } = useSWR(
    shouldFetch
      ? ['territory-geo', geoJsonUrl, codigoTerritorial, entityId]
      : null,
    async ([, url, code, id]) => {
      console.log('[TerritoryMap] useTerritoryGeoData called with:', {
        url,
        code,
        id,
      });
      ...
      if (url && typeof url === 'string' && url.startsWith('http')) {
        try {
          console.log(
            '[TerritoryMap] Attempting to fetch GeoJSON from URL:',
            url
          );
          const res = await fetch(url);
          if (!res.ok)
            throw new Error(
              'Error al cargar la cartografía de la URL provista.'
            );
          ...
          console.log(
            '[TerritoryMap] Falling back to polygon cache since entityId or code is available'
          );
        }
      }
      ...
      if (id || code) {
        try {
          ...
          console.log(
            '[TerritoryMap] Polygons fetched, total count:',
            polygons.length
          );
          ...
          if (id) {
            console.log(
              '[TerritoryMap] Searching for polygon with entityId:',
              id
            );
            const match = polygons.find((poly) => poly.entityId === id);
            if (match && match.coordinates) {
              console.log(
                '[TerritoryMap] Found polygon by entityId:',
                match.entityLabel
              );
              return {
                type: 'Feature',
          ...
          if (code) {
            console.log(
              '[TerritoryMap] Searching for polygon with code:',
              code
            );
            const match = polygons.find((poly) => poly.ineCode === code);
            if (match && match.coordinates) {
              console.log(
                '[TerritoryMap] Found polygon by code:',
                match.entityLabel
              );
              return {
                type: 'Feature',
      ...
      if (url) {
        throw new Error(
          'Cartografía oficial no disponible o en formato externo.'
        );
      }
...
export default function TerritoryMap({
  ...
  const { geoData, error, isLoading } = useTerritoryGeoData(
    geoJsonUrl,
    codigoTerritorial,
    entityId
  );
  ...
  if (!geoJsonUrl && !codigoTerritorial && !entityId) {
    ...
    return (
      <div
        className="absolute inset-0 flex items-center justify-center bg-white/5 backdrop-blur-sm rounded-[3rem] border border-white/10 text-hunter/50 font-bold p-8 text-center"
  ...
  if (error) {
    ...
    return (
      <div
        className="absolute inset-0 flex items-center justify-center bg-white/5 backdrop-blur-sm rounded-[3rem] border border-white/10 text-hunter/50 font-bold p-8 text-center text-xs"
        suppressHydrationWarning
      >
        <p>
          Cartografía oficial de origen externo.
          <br />
          Por favor use el botón de descarga o enlace.
        </p>
      </div>
    );
  }
  ...
  if (isLoading || !geoData) {
    console.log('[TerritoryMap] Loading state:', {
      isLoading,
      hasGeoData: !!geoData,
    });
    return (
      <div
        className="absolute inset-0 flex items-center justify-center bg-white/5 backdrop-blur-sm animate-pulse rounded-[3rem] border border-white/10 text-hunter/80 font-bold"
        suppressHydrationWarning
      ></div>
    );
  }
