...
export function InstitutionView({ entity, claims = EMPTY_CLAIMS }: InstitutionProps) {
    const nombre = entity.label || "Institución";
    const descripcion = entity.description || "Entidad u organismo que forma parte de la estructura institucional.";
    const categoria = entity.aliases?.[0] || "INSTITUCIÓN";

    const jurisdictionClaim = claims.find((c) => {
        const prop = c.property?.label?.toLowerCase() || "";
        return prop.includes("jurisdiccion") || prop.includes("jurisdicción") || prop.includes("alcance") || prop.includes("cobertura");
    });
    const jurisdiccion = jurisdictionClaim?.value_relation?.label || jurisdictionClaim?.value_raw || "Nacional";

    const budgetClaim = claims.find((c) => {
        const prop = c.property?.label?.toLowerCase() || "";
        return prop.includes("presupuesto") || prop.includes("gasto") || prop.includes("financ");
    });
    const presupuesto = budgetClaim?.value_raw || "N/D";

    const partOfClaim = claims.find((c) => {
        const prop = c.property?.label?.toLowerCase() || "";
        return prop.includes("parte de") || prop.includes("depende de") || prop.includes("pertenece");
    });
    const dependencia = partOfClaim?.value_relation?.label || partOfClaim?.value_raw || "Gobierno de Bolivia";
    const dependenciaId = partOfClaim?.value_relation?.$id;

    const headClaim = claims.find((c) => {
        const prop = c.property?.label?.toLowerCase() || "";
        return prop.includes("director") || prop.includes("ministro") || prop.includes("jefe") || prop.includes("autoridad") || prop.includes("responsable");
    });
    const headName = headClaim?.value_relation?.label || headClaim?.value_raw;
    const headId = headClaim?.value_relation?.$id;

    const creado = new Date(entity.$createdAt || Date.now()).toLocaleDateString("es-ES", {
        year: "numeric",
        month: "long",
        day: "numeric",
    });

    const modificado = new Date(entity.$updatedAt || Date.now()).toLocaleDateString("es-ES", {
        year: "numeric",
        month: "long",
        day: "numeric",
    });
    ...
    return (
        <main className="min-h-screen bg-neutral-white text-primary-green font-sans antialiased pb-24">
                ...
                <div className="max-w-6xl mx-auto relative z-10">
                    <div className="flex flex-col items-center text-center">
                        <div className="mb-8 relative group">
                            <div
                                className="w-24 h-24 bg-hunter rounded-[2rem] flex items-center justify-center text-primary-green shadow-2xl rotate-3 group-hover:rotate-0 transition-transform duration-500 border-4 border-white/10"
                            >
                                <Building size={40} strokeWidth={2} />
                            </div>
                            <div
                                className="absolute -bottom-2 -right-2 bg-emerald-500 text-white p-2 rounded-xl shadow-lg ring-4 ring-primary-green"
                            >
                        ...
                        <div className="max-w-3xl space-y-6">
                            ...
                            <p
                                className="text-lg md:text-xl text-hunter/60 font-medium leading-relaxed text-pretty"
                            >
                                {descripcion}
                            </p>
                        </div>
            ...
            <div
                ...
                style={{ animation: "slideUp 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards" }}
            >
                <div
                    className="bg-white p-6 rounded-[2rem] shadow-xl border border-primary-green/5 flex items-center gap-5"
                >
            ...
            <div
                ...
                style={{ animation: "slideUp 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards", animationDelay: "100ms", opacity: 0, animationFillMode: "forwards" }}
            >
                <div className="lg:col-span-8 space-y-8">
                    <div
                        className="bg-white p-10 rounded-[4rem] shadow-2xl shadow-primary-green/5 border border-primary-green/5"
                    >
                        <div className="flex items-center gap-4 mb-10">
                            ...
                            <div>
                                <h3 className="text-2xl font-black tracking-tight" style={{ textWrap: "balance" }}>
                        ...
                        <a
                            href={dependenciaId ? buildPath(`/entity?id=${dependenciaId}`) : "#"}
                        ...
                        >
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-6">
                                    <div
                                        className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-primary-green group-hover:bg-hunter transition-colors shadow-sm"
                                    >
                                        <Globe size={20} />
                                    </div>
                                    <div>
                                        <p
                                            className="text-[9px] font-black opacity-40 uppercase tracking-widest mb-1"
                                        >
                                            Entidad Superior
                                        </p>
                    ...
                    <div
                        className="bg-white p-10 rounded-[4rem] shadow-xl border border-primary-green/5"
                    >
                        <div className="flex items-center gap-4 mb-10">
                            <div
                        ...
                        <div className="space-y-4">
                            {personalRelacionado.length > 0 ? (
                                ...
                                <p className="text-sm font-medium opacity-50 text-center py-4">No se encontro personal vinculado directamente.</p>
                            )}
                        </div>
                    </div>
                </div>
                    ...
                    >
                        <div className="relative z-10">
                            ...
                            <div className="space-y-6">
                                ...
                                <div
                                    className="flex items-start gap-4 p-4 bg-white/50 rounded-2xl border border-primary-green/5"
                                >
                                    <Calendar
                                        size={16}
                        ...
                        <BarChart3
                            className="absolute -right-8 -bottom-8 w-40 h-40 opacity-[0.03] group-hover:rotate-12 transition-transform duration-700"
                        />
                    </div>
                </aside>
            </div>
