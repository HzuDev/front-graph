...
export function TerritoryView({ entity, claims = EMPTY_CLAIMS }: TerritoryProps) {
    ...
    const instanciaClaim = claims.find(c => c.property?.label?.toLowerCase().includes("instancia de"));
    const categoria = instanciaClaim?.value_relation?.label || "Entidad Territorial";

    const poblacionClaim = claims.find(c => c.property?.label?.toLowerCase().includes("poblacion") || c.property?.label?.toLowerCase().includes("población"));
    const poblacionStr = poblacionClaim?.value_raw || "N/D";
    const poblacionNum = parseInt(poblacionStr);
    const poblacionFormat = isNaN(poblacionNum) ? poblacionStr : new Intl.NumberFormat('es-BO').format(poblacionNum);

    let poblacionNota = "Información demográfica";
    if (poblacionClaim && poblacionClaim.qualifiers) {
        const edadQual = poblacionClaim.qualifiers.find(q => typeof q.property === "object" && q.property?.label?.toLowerCase()?.includes("edad"));
    ...
    const territorialCodeClaim = claims.find(c => {
        ...
        return lbl.includes("codigo territorial") || lbl.includes("código territorial");
    });
    ...
    const geoJsonClaim = claims.find(c => {
        ...
        // @ts-ignore
        const lbl = (c.property?.label || "").toLowerCase().trim();
        if (lbl === "territorio" || lbl === "mapa" || lbl.includes("geojson")) return true;
        // Fallback: If it's a URL to the storage bucket
        if (val.includes("appwrite.sociest.org/v1/storage/") || val.includes(".geojson")) {
            return true;
        }
        return false;
    });
    ...
    return (
        <div className="min-h-screen bg-neutral-white text-primary-green antialiased pb-24">
            ...
            <div className="max-w-7xl mx-auto px-6 -mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 relative z-20">
                <div className="bg-white p-8 rounded-[3rem] shadow-2xl shadow-primary-green/5 border border-primary-green/5 flex items-center gap-6 slide-up" style={{ animationDelay: '0.1s' }}>
                    <div className="p-4 bg-primary-green text-hunter rounded-2xl">
                        <Users size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-30">Población Electoral</p>
                        <p className="text-3xl font-black text-primary-green">{poblacionFormat}</p>
                ...
                <div className="bg-white p-8 rounded-[3rem] shadow-2xl shadow-primary-green/5 border border-primary-green/5 flex items-center gap-6 slide-up" style={{ animationDelay: '0.2s' }}>
                    <div className="p-4 bg-primary-green/5 rounded-2xl text-primary-green">
                        <Activity size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-30">Código Territorial</p>
                        <p className="text-3xl font-black">{codigoTerritorial}</p>
                        <p className="text-[9px] font-bold opacity-40 uppercase mt-1">Identificador OEP</p>
                    </div>
                </div>

                <div className="bg-white p-8 rounded-[3rem] shadow-2xl shadow-primary-green/5 border border-primary-green/5 flex items-center gap-6 slide-up" style={{ animationDelay: '0.3s' }}>
                    <div className="p-4 bg-primary-green/5 rounded-2xl text-primary-green">
                        <Landmark size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-30">División Política</p>
                        <p className="text-3xl font-black">{loadingRelated ? "..." : (provinces.length > 0 ? provinces.length : "Sin")} Divisiones</p>
                        <p className="text-[9px] font-bold opacity-40 uppercase mt-1">Nivel Interno</p>
                    </div>
                </div>
            </div>

            <div className="max-w-7xl mx-auto px-6 mt-20 grid grid-cols-1 lg:grid-cols-12 gap-12">
                <div className="lg:col-span-8 space-y-12 slide-up" style={{ animationDelay: '0.4s' }}>
                    ...
                    <section className="space-y-8">
                        ...
                        {loadingRelated ? (
                            <div className="p-12 text-center opacity-50 font-bold animate-pulse">Buscando actores relevantes...</div>
                        ) : candidates.length === 0 ? (
                ...
                <aside className="lg:col-span-4 space-y-8 slide-up" style={{ animationDelay: '0.5s' }}>
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-start gap-4 p-4 bg-white/40 rounded-2xl border border-primary-green/5">
                                    <Calendar size={16} className="shrink-0 mt-0.5 opacity-30 text-primary-green" />
                                    <div>
                                        <p className="text-[9px] font-black opacity-30 uppercase text-primary-green">
                                            Última Modificación
                                        </p>
                                        <p className="text-xs font-bold text-primary-green">
                                            {new Date(entity.$updatedAt || Date.now()).toLocaleDateString("es-BO", { year: "numeric", month: "long", day: "numeric" })}
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    {geoJsonUrl && (
                        <div className="bg-primary-green text-hunter p-10 rounded-[3.5rem] shadow-2xl relative overflow-hidden group">
                            ...
                            <p className="text-xs font-medium opacity-50 mb-8 relative z-10 leading-relaxed text-pretty">
                                Accede a los archivos geoespaciales oficiales del {nombre} para análisis externo.
                            </p>
                            <a
                                href={geoJsonUrl}
                                target="_blank"
