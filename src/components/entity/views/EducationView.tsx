...
export function EducationView({ entity, claims = EMPTY_CLAIMS }: EducationProps) {
    // Basic entity data
    ...
    const fechaCreacion = new Date(entity.$createdAt || Date.now()).toLocaleDateString("es-ES", {
        year: "numeric",
        month: "long",
        day: "numeric",
    });

    const fechaModificacion = new Date(entity.$updatedAt || Date.now()).toLocaleDateString("es-ES", {
        year: "numeric",
        month: "long",
        day: "numeric",
    });
    ...
    const vinculos = claims.filter(c => {
        const prop = c.property?.label?.toLowerCase() || "";
        return !prop.includes("instancia") && !prop.includes("logo") && (c.value_relation || c.subject?.$id !== entity.$id);
    }).map(c => {
        const isIncoming = c.value_relation?.$id === entity.$id;
        const relatedEntity = isIncoming ? c.subject : c.value_relation;

        let relacion = c.property?.label || "Vínculo";
        ...
        if (c.qualifiers && c.qualifiers.length > 0) {
            ...
            const detailQ = c.qualifiers.find(q => {
                const p = q.property?.label?.toLowerCase() || "";
                return p.includes("titulo") || p.includes("título") || p.includes("carrera") || p.includes("grado");
            });
        ...
        return {
            ...
            detalle: detalle,
            fechaFin: fechaFin,
            link: relatedEntity?.$id ? buildPath(`/entity?id=${relatedEntity.$id}`) : "#",
            esPersona: typeof relatedEntity?.label === "string" && relatedEntity.label.includes(" ") // check if label contains space (naive heuristic for person names)
        };
    });
    ...
    return (
        <main className="min-h-screen bg-neutral-white text-primary-green font-sans antialiased pb-24">
            <section className="bg-primary-green pt-32 pb-24 px-6 relative overflow-hidden text-center">
                ...
                >
                    <div className="mb-10 relative group">
                        <div
                            className="w-44 h-44 bg-white rounded-full flex items-center justify-center shadow-2xl p-4 border-4 border-hunter/20 overflow-hidden relative"
                        >
                            {logoUrl ? (
                                <img
                                    src={logoUrl}
                    ...
                    <div className="max-w-4xl space-y-6">
                        ...
                        {alias && (
                            <div
                                className="flex justify-center items-center gap-4 text-2xl font-bold text-hunter/40 italic"
                            >
                                <span>{alias}</span>
                            </div>
                        )}
            ...
            <div
                className="max-w-6xl mx-auto px-6 mt-16 grid grid-cols-1 lg:grid-cols-12 gap-12"
            >
                <div className="lg:col-span-8 space-y-8">
                    <div
                        className="bg-white p-12 rounded-[4rem] shadow-xl border border-primary-green/5 relative overflow-hidden"
                    >
                        ...
                        {vinculos.length > 0 ? (
                                        ...
                                        >
                                            <div className="flex items-center gap-6">
                                                <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-primary-green shadow-sm group-hover:bg-hunter transition-colors">
                                                    {v.esPersona || v.relacion.toLowerCase().includes("estudi") ? (
                            ...
                            <div className="text-center p-8 bg-primary-green/5 rounded-[3rem]">
                                <p className="text-sm font-bold opacity-40 uppercase tracking-widest">No hay vínculos registrados</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <aside className="lg:col-span-4 space-y-8">
                    ...
                    >
                        <div className="relative z-10">
                            ...
                            <div className="space-y-8">
                                <div>
                                    <p
                                        className="text-[9px] font-black opacity-30 uppercase mb-2"
                                    >
                                        Código de Identificación
                                    </p>
                                    <p
                                        className="text-xs font-mono font-bold break-all opacity-70 leading-relaxed"
                                    >
                                        {idRegistro}
                                    </p>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div
                                        className="flex items-center gap-4 p-4 bg-white/40 rounded-2xl border border-primary-green/5"
                                    >
                                        <Calendar
                                            size={16}
                    ...
                    <div
                        className="bg-primary-green text-hunter p-8 rounded-[3.5rem] shadow-2xl relative overflow-hidden group"
                    >
                        <h4
                        ...
                        >
                            Navega a través de los vínculos académicos registrados
                            para descubrir la trayectoria de los actores públicos.
                        </p>
                        <a
                            href="/search"
