...
const getIconForPropertyStatic = (label: string) => {
  ...
  if (
    lowerLabel.includes("lugar") ||
    lowerLabel.includes("ubicación") ||
    lowerLabel.includes("municipio")
  )
    return <MapPin className="w-4 h-4" />;
  if (
    lowerLabel.includes("web") ||
    lowerLabel.includes("url") ||
    lowerLabel.includes("sitio")
  )
    return <Globe className="w-4 h-4" />;
  if (
    lowerLabel.includes("organización") ||
    lowerLabel.includes("partido") ||
    lowerLabel.includes("institución")
  )
...
const EntityDetailComponent: React.FC<EntityDetailProps> = ({
  entity,
  claims,
}) => {
  ...
  const {
    outgoingClaims,
    incomingClaims,
    groupedOutgoingClaims,
    groupedIncomingClaims,
  } = useMemo(() => {
    ...
    const groupedOutgoing = outgoing.reduce(
      (acc, claim) => {
        const propertyLabel =
          typeof claim.property === "object"
            ? (claim.property as any).label
    ...
    const groupedIncoming = incoming.reduce(
      (acc, claim) => {
        const propertyLabel =
          typeof claim.property === "object"
            ? (claim.property as any).label
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                {entity.aliases &&
                  ...
                  entity.aliases.map((alias, index) => (
                    <span
                      key={index}
                      className="px-3 py-1 bg-hunter/10 text-hunter/80 rounded-full text-xs font-medium backdrop-blur-sm border border-hunter/10"
                    >
                      {alias}
                    </span>
                  ))}
          ...
          <div className="p-8 md:p-12">
            ...
            {outgoingClaims.length > 0 && (
              <section className="mb-12">
                ...
                <div className="space-y-4">
                  {Object.entries(groupedOutgoingClaims).map(
                    ([property, claimsList]) => (
                      <PropertyCard
                        key={property}
            ...
            {incomingClaims.length > 0 && (
              <section>
                ...
                <div className="space-y-4">
                  {Object.entries(groupedIncomingClaims).map(
                    ([property, claimsList]) => (
                      <PropertyCardIncoming
                        key={property}
                        propertyLabel={property}
                        claimCount={claimsList.length}
                        claimsList={claimsList}
            ...
            {outgoingClaims.length === 0 && incomingClaims.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-primary-green/40 bg-[#f8f9fa] rounded-2xl border-dashed border-2 border-primary-green/10">
                ...
                <p className="text-sm">
                  Esta entidad aún no tiene propiedades o relaciones registradas
                </p>
              </div>
            )}
          </div>
  ...
  <div className="bg-[#f8f9fa] rounded-2xl p-6 border border-primary-green/5 hover:border-primary-green/20 transition-all">
    ...
    <div className="space-y-3">
      {claimsList.map((claim) => (
        <div
          key={claim.$id}
          className="text-primary-green text-base leading-relaxed"
        >
