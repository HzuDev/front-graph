...
import { MapPin, UserCheck, ExternalLink } from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row gap-12 items-center md:items-end">
          <div className="relative group">
            <div className="w-56 h-64 bg-hunter rounded-[3.5rem] overflow-hidden shadow-2xl border-4 border-white/10 flex items-center justify-center">
              {(() => {
                const portraitClaim = claims.find((c) => {
                  const prop = normalizeText(extractLabel(c.property));
                  return c.datatype === "image" && (prop.includes("foto") || prop.includes("imagen") || prop.includes("retrato") || prop.includes("perfil"));
                }) || claims.find(c => c.datatype === "image" && !normalizeText(extractLabel(c.property)).includes("logo"));
                ...
                return (
                  <div className="w-full h-full bg-linear-to-br from-hunter to-[#e6e0a8] flex flex-col items-center justify-center p-8 text-center">
                    <span className="text-primary-green font-black text-6xl mb-4">
                      {mainParty?.sigla || (entityLabel?.charAt(0) || "P")}
                    </span>
                    <p className="text-primary-green/40 text-[10px] font-black uppercase tracking-[0.2em] leading-tight">
                      {mainParty?.nombre || "Perfil Verificado"}
                    </p>
                  </div>
                );
              })()}
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-8">
              <div className="flex items-center gap-3 text-hunter/60 font-bold group cursor-pointer hover:text-hunter transition-colors">
                ...
                {lugarNacimientoId ? (
                  ...
                  >
                    {lugarNacimiento} <ExternalLink size={12} className="opacity-50" />
                  </a>
                ) : (
