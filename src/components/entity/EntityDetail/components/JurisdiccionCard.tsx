    ...
    <div className="bg-white p-10 rounded-[3.5rem] border border-primary-green/5 shadow-sm relative overflow-hidden group">
      <div className="relative z-10">
        ...
        <div className="space-y-6">
          <h5 className="text-3xl font-black tracking-tighter leading-none">
            {lugarNacimiento}
          </h5>
          <p className="text-sm font-medium text-primary-green/60 text-pretty">
            Área administrativa vinculada a la trayectoria del candidato.
          </p>
          <a
            href={buildPath('/mapa')}
            className="inline-flex items-center gap-3 text-xs font-black uppercase tracking-widest text-primary-green border-b-2 border-primary-green/10 hover:border-primary-green transition-all pb-1"
          >
            Ver estadísticas regionales <ChevronRight size={14} />
          </a>
        </div>
      </div>
      <Globe className="absolute -right-10 -bottom-10 w-44 h-44 opacity-[0.03] group-hover:rotate-12 transition-transform duration-700" />
    </div>
  );
};
