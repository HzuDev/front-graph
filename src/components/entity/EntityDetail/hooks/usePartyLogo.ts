...
export const usePartyLogo = (partyId: string | undefined, existingLogo: string | null | undefined) => {
    ...
    if (!existingLogo && partyId && !fetchedLogo) {
      databases.listDocuments<Claim>(DATABASE_ID, COLLECTIONS.CLAIMS, [
        ...
        if (res.documents && res.documents.length > 0) {
          const logoClaim = res.documents.find(c => {
            const label = typeof c.property === 'object' ? c.property?.label?.toLowerCase() || '' : '';
            return label.includes('logo') || label.includes('emblema') || label.includes('organizacion');
          }) || res.documents[0];
