...
import { getQualifierEntity, getQualifierValueByLabels, getEntityId, extractLabel } from "./utils/claimHelpers";
...
export const EntityDetail: React.FC<EntityDetailProps> = ({
  ...
  const partyQualifierEntity = candidateClaim ? getQualifierEntity(candidateClaim, "partido/movimiento politico") : null;
  const partyQualifierRole = candidateClaim ? getQualifierValueByLabels(candidateClaim, ["rol", "titular", "cargo"]) || null : null;

  const partyLogo = (claims || []).find(c => {
    const propLabel = normalizeText(extractLabel(c.property));
    return c.datatype === 'image' && (propLabel.includes('logo') || propLabel.includes('emblema') || propLabel.includes('organizacion'));
  ...
  const mainParty = partyQualifierEntity ? {
    ...
    nombre: extractLabel(partido[0].value_relation || partido[0].value_raw),
    sigla: getSigla(extractLabel(partido[0].value_relation || partido[0].value_raw)),
    rol: extractLabel(partido[0].property) || "Miembro",
    logo: partyLogo || null,
  } : null;

  const fetchedLogo = usePartyLogo(mainParty?.id || undefined, partyLogo);
  const mainPartyWithLogo = mainParty ? { ...mainParty, logo: mainParty.logo || fetchedLogo } : null;
  ...
  return (
    <div className="min-h-screen bg-neutral-white text-primary-green font-sans antialiased pb-24">
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-12 grid grid-cols-1 lg:grid-cols-12 gap-10 relative z-20">
        ...
        <aside className="lg:col-span-4 space-y-8 animate-slide-up" style={{ animationDelay: "0.2s" }}>
