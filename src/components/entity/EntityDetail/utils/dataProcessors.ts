...
import type { TimelineItem, EducationItem, LinkItem, SocialLink } from "../types";
import {
  normalizeText,
} from "./formatters";
import {
...
export const processClaimsData = (claims: Claim[], entity: Entity) => {
  ...
  const getRelatedEntityForClaim = (claim: Claim) => {
    ...
    if (claim.subject && typeof claim.subject !== "string") return claim.subject;
  ...
  const isLeadershipClaim = (claim: Claim) => {
    const propertyLabel = normalizeText(extractLabel(claim.property));
    const roleQualifier = getQualifierValueByLabels(claim, ["titular", "presidente", "fundador", "lider"]) || "";
    return (
  ...
  (claims || []).forEach((claim) => {
    const propertyLabel =
      typeof claim.property === "object"
        ? (claim.property as any).label?.toLowerCase() || ""
        : "";

    const valueRaw = claim.value_raw || "";
    const valueLabel = claim.value_relation ? ((claim.value_relation as any).label?.toLowerCase() || "") : "";
    ...
    if (claim.datatype === 'url' || propertyLabel.includes('url') || propertyLabel.includes('red social')) {
      ...
      if (urlLower.includes("facebook")) {
        ...
        return;
      } else if (propertyLabel.includes("web") || propertyLabel.includes("sitio")) {
    ...
    if (
      ...
      trayectoriaItems.push({
        ...
        entidadId,
        inicio: getQualifierValueByLabels(claim, ["fecha de inicio", "inicio"]) || null,
    ...
    if (
      ...
      const universidadEntity = getQualifierEntity(claim, "universidad") || claim.value_relation;
      const universidad = universidadEntity ? extractLabel(universidadEntity) : null;
      const universidadId = getEntityId(universidadEntity);

      educacionItems.push({
        id: claim.$id,
        titulo: gradoAcademico || claim.value_raw || extractLabel(claim.value_relation) || "Formación académica",
        universidad: universidad && !isIdLike(universidad) ? universidad : null,
        universidadId,
        fecha: getQualifierValueByLabels(claim, ["fecha de fin", "fecha de inicio"]) || null,
      });
      usedClaims.add(claim.$id);
      return;
    }
    ...
    // Lugar de Nacimiento (Birthplace) / Origin
      ...
      lugarNacimiento = extractLabel(claim.value_relation) || claim.value_raw || lugarNacimiento;
      lugarNacimientoId = getEntityId(claim.value_relation) || lugarNacimientoId;
    }
    // Other links/connections (Vinculos/Ecosistema)
    if (isSurveyClaim(claim) || normalizeText(extractLabel(claim.property)).includes("resultado")) {
    ...
    if (!usedClaims.has(claim.$id)) {
      ...
      const relatedEntity = getRelatedEntityForClaim(claim);
      const detalle = extractLabel(relatedEntity) || getReadableValue(claim.value_raw) || "Detalle";
      ...
      if (isIdLike(detalle)) {
        ...
        if (fixed) {
          vinculosItems.push({
            ...
            relacion: references[0]?.label || getQualifierValueByLabels(claim, ["titular", "presidente", "fundador"]) || null,
            referencias: references,
          });
          return;
        }
  ...
  if (lugarNacimiento === "Ubicación desconocida") {
    const territoryQualifierEntity = (claims || [])
      .map((claim) => getQualifierEntity(claim, "lugar de nacimiento") || getQualifierEntity(claim, "territorio"))
      .find((value) => !!value);

    if (territoryQualifierEntity) {
      lugarNacimiento = extractLabel(territoryQualifierEntity) || lugarNacimiento;
      lugarNacimientoId = getEntityId(territoryQualifierEntity) || lugarNacimientoId;
    } else {
      const territoryQualifierRaw = (claims || [])
        .map((claim) => getQualifierValueByLabels(claim, ["lugar de nacimiento", "territorio"]))
        .find((value) => !!value);
  ...
  if (lugarNacimiento === "Ubicación desconocida") {
    const roleWithLocation = trayectoriaItems.find((item) => {
      ...
      return raw.includes("la paz") || raw.includes("el alto") || raw.includes("cochabamba") || raw.includes("santa cruz");
    });
    if (roleWithLocation) {
      if ((roleWithLocation.cargo || "").toLowerCase().includes("la paz")) { lugarNacimiento = "Departamento de La Paz"; lugarNacimientoId = "698c9f28d8defbb3b4cf"; } // Use known ID for La Paz or let it be generic
      else if ((roleWithLocation.cargo || "").toLowerCase().includes("cochabamba")) lugarNacimiento = "Departamento de Cochabamba";
      else if ((roleWithLocation.cargo || "").toLowerCase().includes("santa cruz")) lugarNacimiento = "Departamento de Santa Cruz";
    }
  }
