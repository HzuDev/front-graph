...
export const useMapGeometry = () => {
    ...
    useEffect(() => {
        ...
        const loadPolygons = async () => {
            ...
            console.log('[useMapGeometry] 🚀 Iniciando carga de geometrías del mapa...');
            ...
            try {
                ...
                if (!isMounted) {
                    console.log('[useMapGeometry] ⚠️ Componente desmontado, cancelando...');
                    return;
                }

                console.log('[useMapGeometry] 📦 Recibidos ' + polygons.length + ' polígonos de fetchPolygons');
                ...
                console.log('[useMapGeometry] ✅ ' + validPolygons.length + ' polígonos válidos (nivel 3 - municipios)');
                ...
                if (featuresWithoutName.length > 0) {
                    console.warn('[useMapGeometry] ⚠️ ' + featuresWithoutName.length + ' features sin nombre');
                }
                ...
                if (featuresWithInvalidCoords.length > 0) {
                    console.warn('[useMapGeometry] ⚠️ ' + featuresWithInvalidCoords.length + ' features con coordenadas inválidas');
                }
                ...
                console.log('[useMapGeometry] 🎉 Geometrías listas para renderizar (' + Math.round(duration) + 'ms)');
