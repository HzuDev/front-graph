...
const MapViewLeaflet: React.FC<MapViewProps> = ({ onMunicipalitySelect, onMapReset, selectedEntityId }) => {
    ...
    const { selectedFeatureId, userDetectedFeatureId, userDetectedFeatureName } = selectionState;
    ...
    const { onEachFeature } = useMapEventHandlers(
        ...
        (id: string | null) => setSelectionState(prev => ({ ...prev, selectedFeatureId: id })),
        setHoveredFeatureId
    );
        ...
        return (
            <div className="absolute inset-0 flex items-center justify-center bg-black rounded-[2.5rem]">
                <div className="text-center">
                    ...
                    <p className="text-sm text-gray-400 font-medium">Cargando minimapa...</p>
                    <p className="text-xs text-gray-500 mt-2">Preparando polígonos municipales</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="absolute inset-0 rounded-[2.5rem] overflow-hidden">
            ...
            <div className="absolute bottom-4 left-4 bg-black/80 backdrop-blur-sm border border-white/5 rounded-xl p-3 shadow-xl z-999">
                <p className="text-xs font-bold mb-2 text-gray-300">Niveles Administrativos</p>
