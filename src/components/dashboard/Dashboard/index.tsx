...
import {
    MapPin, Layers,
    ChevronRight, ArrowUpRight, Database,
    ListFilter, Globe, X, Info
} from 'lucide-react';
...
const EntityDashboard: React.FC = () => {
    ...
    } = useDashboardState(null, userMunicipalityName, userLocation);
        ...
        <div className="min-h-screen bg-neutral-white text-primary-green font-sans selection:bg-primary-green selection:text-hunter">
            ...
            <header className="fixed top-0 w-full z-50 bg-neutral-white/80 backdrop-blur-md border-b border-primary-green/5 px-6 py-4 flex justify-between items-center">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="font-black text-lg leading-none tracking-tight">MONITOR</h1>
                        <p className="text-[10px] uppercase tracking-widest font-bold opacity-40">Elecciones 2026</p>
                    </div>
                </div>

                <nav className="hidden md:flex items-center gap-6">
                    <a href='/' className="text-sm font-bold opacity-50 hover:opacity-100 transition-opacity hover:underline">Inicio</a>
                    <a href='/mapa' className="text-sm font-bold opacity-50 hover:opacity-100 transition-opacity hover:underline">Mapa Global</a>
                </nav>
                    ...
                    <div className="bg-primary-green/5 border-2 border-primary-green/20 rounded-2xl p-4 shadow-lg backdrop-blur-sm flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-primary-green/60">
                                Configura tu ubicación para ver información relevante de tu municipio
                            </p>
                        </div>
                        <a
                            href="/onboarding/1"
                        ...
                        <div className="relative z-50">
                            <SearchCommand
                                onSelect={useCallback((entity) => {
                                    window.location.href = `/entity?id=${entity.$id}`;
                                }, [])}
                            />
                        </div>
                            ...
                            {FILTERS.map((f) => (
                                <button
                                    ...
                                    className={`px-5 py-2 rounded-full text-xs font-bold transition-all ${
                                        'bg-white border-2 border-transparent text-primary-green/40 hover:border-primary-green/10'
                                    }`}
                                >
                                    {f}
                                </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-7 space-y-6">
                                ...
                                {userMunicipalityName ? (
                                    <div className="bg-green-500/10 border-2 border-green-500/30 rounded-2xl px-4 py-3 mb-2">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <p className="text-xs text-green-700 font-bold">Tu ubicación detectada</p>
                                        </div>
                        ...
                        {loading ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {[1, 2, 3, 4].map((i) => (
                                    <div key={i} className="bg-white border border-primary-green/5 p-6 rounded-[2rem] animate-pulse">
                        ...
                        <div className="bg-primary-green text-hunter p-8 rounded-[2.5rem] mt-12 grid grid-cols-3 gap-8">
                            <div>
                                <p className="text-[10px] uppercase font-bold opacity-50 tracking-widest mb-1">Entidades</p>
                                <p className="text-3xl font-black">
                                    {loading ? '...' : formatNumber(stats.entities)}
                                </p>
                            </div>
                            <div>
                                <p className="text-[10px] uppercase font-bold opacity-50 tracking-widest mb-1">Declaraciones</p>
                                <p className="text-3xl font-black">
                                    {loading ? '...' : formatNumber(stats.claims)}
                                </p>
                            </div>
                            <div className="relative">
                                <p className="text-[10px] uppercase font-bold opacity-50 tracking-widest mb-1">Propiedades</p>
                                <p className="text-3xl font-black">
                    ...
                    <div className="lg:col-span-5">
                        <div className="sticky top-28">
                            <div className="flex justify-between items-end mb-4">
                                <h3 className="font-black text-xs uppercase tracking-[0.2em] opacity-30">Vista de Mapa</h3>
                                <div className="flex gap-2">

                                    <Button variant="secondary" size="sm" title="Ver mapa completo" href='/mapa' icon={<Layers size={16} />} className="group-hover:scale-110 transition-transform" />
                                </div>
                            </div>
                            ...
                            <div className="aspect-4/5 bg-hunter rounded-[2.5rem] border-2 border-primary-green relative overflow-hidden shadow-2xl shadow-primary-green/10">
                                ...
                                <div className="absolute inset-0">
                                    <MapViewWrapper
                                        ...
                                        onMunicipalitySelect={useCallback((municipality: { name: string; department: string; entityId: string }) => {
                                            setSelectedMunicipality({
                                ...
                                <div className="absolute bottom-6 left-6 right-6 bg-white/90 backdrop-blur-md p-6 rounded-3xl border border-primary-green/10 shadow-lg z-10 pointer-events-none">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <span className="font-bold text-sm tracking-tight">
                                            {selectedMunicipality?.name || userMunicipalityName || 'Bolivia'}
                                            {(selectedMunicipality?.department || (userMunicipalityName && 'Bolivia')) && `, ${selectedMunicipality?.department || 'Bolivia'}`}
                                        </span>
                                        {userLocation && (selectedMunicipality || userMunicipalityName) && (
                                    ...
                                    {municipalityEntityId ? (
                                        <a
                                            href={`/entity?id=${municipalityEntityId}`}
            ...
            <footer className="border-t border-primary-green/5 py-12 px-6">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
                    <p className="text-[10px] font-black text-primary-green/20 uppercase tracking-[0.4em]">Plataforma Cívica Independiente</p>
                    <div className="flex gap-8">
                        <span className="text-[10px] font-bold opacity-30">GraphQL API</span>
